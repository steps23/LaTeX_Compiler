...
- Desktop IPC exposes versioned runtime/storage info, bounded project/file persistence commands, local TeX runtime diagnostics and global selected-runtime persistence.
...
- Desktop builds can compile with the selected local TeX runtime through bounded Tauri commands; browser builds continue to use the HTTP fallback compiler.
...
- Local TeX runtime detection, selected-runtime persistence, per-project runtime overrides and local desktop compilation exist; custom runtime paths and package management are deferred.
