...
`src-tauri/src/lib.rs` exposes runtime/storage introspection, TeX runtime diagnostics and selection, local TeX compilation, SyncTeX forward/reverse lookup, one-shot texlab completion lookup plus bounded project-storage commands: `detect_tex_runtimes`, `get_tex_runtime_selection`, `save_tex_runtime_selection`, `compile_latex_project`, `cancel_latex_compile`, `query_synctex_forward`, `query_synctex_reverse`, `query_latex_lsp_completions`, `get_all_projects`, `get_project`, `save_project`, `delete_project`, `get_project_files`, `get_file`, `save_file` and `delete_file`. All native storage is constrained to Tauri's app data directory and all compile/LSP workspaces/artifacts are constrained to Tauri's app cache directory. Project IDs and project-relative paths are validated before disk access. Runtime selection is persisted as app-owned JSON and stores only the selected detected runtime ID/bin directory. Local compilation invokes an explicit detected TeX executable selected from the project engine allowlist with fixed arguments including `-synctex=1` through Rust `Command`, rejects oversized input/file sets, supports job-ID cancellation, writes stdout/stderr to app-owned log files to avoid pipe backpressure, caps returned log/PDF sizes and removes failed/transient workspaces. Successful SyncTeX artifacts stay in app cache by artifact ID for bounded `synctex view` and `synctex edit` lookup. The PDF preview maps double-click coordinates to SyncTeX top-left big-point coordinates and jumps Monaco to the returned project-relative source line. The LSP bridge writes a bounded project snapshot to app cache, starts the selected detected `texlab` executable over stdio, sends LSP `initialize`, `textDocument/didOpen` and `textDocument/completion`, parses capped JSON-RPC responses, then shuts the server down; no global filesystem permission or generic shell command exists.

`src-tauri/src/tex_runtime.rs` probes process `PATH` and known platform TeX bin directories. It preserves executable names/symlinks for TeX engines, runs only allowlisted TeX binaries with separated `--version` arguments, applies a timeout and output cap, and returns diagnostics. It does not scan whole disks, mutate system PATH, install packages, invoke `sudo`, or compile user projects.
